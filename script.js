...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!storeList) {
        console.error("Error: store-list element not found in DOM.");
        return;
    }
    ...
    // Fix for dropdown population
    const storeSelect = document.getElementById("store-select");
    if (!storeSelect) {
        console.error("Error: store-select element not found in DOM.");
        return;
    }
    ...
    // Add each store as an option
    stores.forEach(store => {
        const option = new Option(store.name, store.id);
        storeSelect.add(option);
    });

    // Log to confirm options are added
    console.log("Dropdown options:", Array.from(storeSelect.options).map(opt => opt.text));
});
